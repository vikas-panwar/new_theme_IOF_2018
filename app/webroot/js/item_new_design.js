...
$(document).ready(function () {
    ...
    $(document).on("click","#couponApply",function () {
        var coupon_code = $(".cp-code-input").val();
        reloadOrderCart(coupon_code);
    });
    ...
    function reloadOrderCart(coupon_code) {
        $.ajax({
            type: 'post',
    ...
    $(document).on('submit', ".IOF-THEME-11 #CartInfoItemsForm", function(e) {
        if (!document.getElementById("editOrderDetail")) {
            e.preventDefault();
            $("#errorPop").modal('show');
            $("#errorPopMsg").html("Please choose an order type and click save.");
        }
    });
