...
class WebServicesController extends ServicesAppController
{
        ...
        if (isset($headers['merchant_id']) && !empty($headers['merchant_id'])) {
            ...
            if (!empty($merchantCheck)) {
                ...
                foreach ($locStoreList as $k => $locStoreDet) {
                    ...
                    // for delivery
                    $orderType = 3;
                    $finaldata = $this->Common->getNextDayTimeRange($current_date, $today, $orderType, $merchant);
...
//                        $store = $this->Store->find('first', array('conditions' => array('Store.id' => $store_id, 'Store.merchant_id' => $merchant_id, 'Store.is_active' => 1, 'Store.is_deleted' => 0)));
                        $store = $this->Store->find('first', array('conditions' => array('Store.id' => $store_id, 'Store.merchant_id' => $merchant_id, 'Store.is_active' => 1, 'Store.is_deleted' => 0), 'fields' => array('Store.id', 'Store.notification_type', 'Store.notification_email', 'Store.email_id', 'Store.store_name', 'Store.address', 'Store.city', 'Store.state', 'Store.zipcode', 'Store.phone', 'Store.notification_number', 'is_booking_open', 'calendar_limit', 'dineinblackout_limit', 'twilio_api_key', 'twilio_api_token', 'twilio_number', 'store_url')));
                                ...
                                if (!empty($requestBody)) {
                                    if (!empty($requestBody['reservation_date'])) {
                                        ...
                                        if ($save_result) {
                                            ...
                                            $emailSuccess = $this->DefaultTemplate->find('first', array('conditions' => array('DefaultTemplate.template_code' => $template_type, 'DefaultTemplate.is_default' => 1), 'fields' => array('DefaultTemplate.id', 'DefaultTemplate.template_message', 'DefaultTemplate.template_subject', 'sms_template')));
            ...
            if (!empty($merchantCheck)) {
                if (isset($requestBody['store_id']) && !empty($requestBody['store_id'])) {
                    ...
                    if (!empty($storeResult)) {
                        ...
                        $categoryList = $this->Category->find('all', array('conditions' => array('Category.store_id' => $store_id, 'Category.is_active' => 1, 'Category.is_deleted' => 0), 'order' => array('position' => 'asc'), 'recursive' => 2));
                        $respon = array();
                        if ($categoryList) {
                                                    ...
                                                    if ($promotionsList['is_time'] == 1) {
                                                        if (($promotionsList['offer_start_time'] <= $selectedTime) && ($promotionsList['offer_end_time'] >= $selectedTime)) {
                                                            ...
                                                            if (!empty($promotionsList['offer_end_date'])) {
                                                                $promoDateArr = explode('-', $promotionsList['offer_end_date']);
                                            ...
                                            if (!empty($productInfo['ItemOffer'])) {
                                                ...
                                                foreach ($productInfo['ItemOffer'] as $o => $extendedOfferList) {
                                                    ...
                                                    if (!empty($extendedOfferList['end_date'])) {
                                                        $extendedDateArr = explode('-', $extendedOfferList['end_date']);
...
//                                pr($responsedata);
    ...
    public function getToppingPrice($sizeID = null, $ItemId = null, $storeId = null)
    {
        ...
        $Toppingprice = $this->Topping->find('all', array('recursive' => 3, 'fields' => array('id', 'price', 'item_id', 'name', 'category_id', 'is_addon_category', 'addon_id', 'no_size', 'size_id', 'min_value', 'max_value'), 'conditions' => array('Topping.item_id' => $ItemId, 'Topping.store_id' => $storeId, 'Topping.is_active' => 1, 'Topping.is_deleted' => 0, 'Topping.is_addon_category' => 1)));
        return $Toppingprice;
    }
                    ...
                    if (isset($requestBody['store_id']) && !empty($requestBody['store_id'])) {
                        if (isset($requestBody['coupon_code']) && !empty($requestBody['coupon_code'])) {
                            ...
                            if (!empty($storeResult)) {
                                ...
                                $coupon = $this->Coupon->find('first', array('conditions' => array('Coupon.store_id' => $store_id, 'Coupon.coupon_code' => $couponCode, 'Coupon.is_deleted' => 0)));
                                //print_r($coupon);
                                if (!empty($coupon)) {
                                    if ($coupon['Coupon']['is_active'] == 1) {

                                        $currentdatetemp = $this->Webservice->getcurrentTime($store_id, 1);
...
//                    $userDet = $this->User->find('first', array('conditions' => array('User.id' => $user_id, 'User.merchant_id' => $merchant_id, 'User.is_active' => 1, 'User.is_deleted' => 0, 'User.role_id' => $roleid), 'fields' => array('User.id', 'User.role_id', 'User.store_id', 'User.merchant_id', 'User.email', 'User.is_active', 'User.is_deleted')));
                    ...
                    if (isset($requestBody['store_id']) && !empty($requestBody['store_id'])) {
                        ...
                        if (!empty($storeResult)) {
                            ...
                            $UserCoupon = $this->UserCoupon->find('all', array('conditions' => array('UserCoupon.user_id' => $user_id, 'Coupon.store_id' => $store_id, 'UserCoupon.is_active' => 1, 'UserCoupon.is_deleted' => 0, 'Coupon.start_date <= ' => $today, 'Coupon.end_date >= ' => $today, 'Coupon.is_active' => 1, 'Coupon.is_deleted' => 0)));
                            $merchantResult = $this->Merchant->find('first', array('conditions' => array('Merchant.id' => $merchant_id)));
                            $domain = $merchantResult['Merchant']['domain_name'];
                            //print_r($UserCoupon);
                            $protocol = 'http://';
...
//                        pr($couponsDet);
                            ...
                            $promotionList = $this->ItemOffer->find('all', array('recursive' => 2, 'conditions' => array('ItemOffer.is_active' => 1, 'ItemOffer.is_deleted' => 0, 'ItemOffer.store_id' => $store_id, 'ItemOffer.start_date <= ' => $today, 'ItemOffer.end_date >= ' => $today)));
                            ...
                            $offerList = $this->Offer->find('all', array(
                                'recursive' => 2,
                ...
                if (isset($headers['user_id']) && !empty($headers['user_id'])) {
                    ...
                    if (!empty($userResult)) {
                        ...
                        if (isset($requestBody['fname']) && !empty($requestBody['fname']) && isset($requestBody['phone']) && !empty($requestBody['phone']) && isset($requestBody['country_code_id']) && !empty($requestBody['country_code_id'])) {
                            ...
                            $data['User']['city_id'] = 0;
                            $data['User']['zip_id'] = 0;
                            $merchantResult = $this->Merchant->find('first', array('conditions' => array('Merchant.id' => $merchant_id), 'fields' => array('name', 'email', 'address', 'state', 'city', 'zipcode', 'phone', 'domain_name')));
...
//                            if ($requestBody['is_sms_check'] == 1) {
                                ...
                                if (!empty($userResultDet['User']['country_code_id'])) {
                                    $countryCode = $this->CountryCode->find('first', array('conditions' => array('CountryCode.id' => $userResultDet['User']['country_code_id']), 'fields' => array('id', 'code')));
        ...
        if (isset($headers['merchant_id']) && !empty($headers['merchant_id'])) {
            ...
            if (!empty($merchantCheck)) {
                ...
                if (isset($headers['user_id'])) {
                    ...
                    $zoneData = $this->Common->addressInZone($DelAddress, $requestBody['store_id'], "mob");
                    if (empty($zoneData)) {
                                    ...
                                    if ($requestBody['is_future_order'] == 1) {
                                        ...
                                        if (!empty($result[1])) {
                                            if (($result[0] == 'paypal') || ($result[0] == 'creditcard')) {
                                                ...
                                                if (isset($requestBody['coupon_code']) && !empty($requestBody['coupon_code'])) {
                                                    $CouponCode = $this->Coupon->find('first', array('conditions' => array('Coupon.coupon_code' => $requestBody['coupon_code'], 'Coupon.is_active' => 1, 'Coupon.is_deleted' => 0), 'fields' => array('id', 'number_can_use', 'used_count', 'discount', 'discount_type')));
                                                    $cudata['Coupon']['used_count'] = $CouponCode['Coupon']['used_count'] + 1;
                                                    $cudata['Coupon']['id'] = $CouponCode['Coupon']['id'];
                                                    $this->Coupon->saveCoupon($cudata);
                                                }
                        ...
                        if (!empty($myFav)) {
                            foreach ($myFav as $key => $mf) {
                                if (!empty($mf['Order']['order_number'])) {
                                    ...
                                    $my_favorites[$key]['placed_date'] = date('m-d-Y h:m a', strtotime($mf['Order']['created']));
                                    ...
                                    if (!empty($mf['Order']['OrderItem'])) {
                                        foreach ($mf['Order']['OrderItem'] as $key1 => $mfo) {
                                            ...
                                            $FreeItem = $this->OrderItemFree->find('first', array('fields' => array('id', 'item_id', 'free_quantity'), 'conditions' => array('OrderItemFree.item_id' => $mfo['item_id'], 'OrderItemFree.order_id' => $mf['Favorite']['order_id'], 'OrderItemFree.is_active' => 1, 'OrderItemFree.is_deleted' => 0)));
    ...
    function convertOrderArr($requestBody = array(), $storeId, $merchant_id, $user_id)
    {
        ...
        foreach ($requestBody['cart_item_list'] as $i => $item_Detail) {
            ...
            if (!empty($freeQuantity)) {
                $itemPricefree = $this->Webservice->getItemPrice($storeId, $merchant_id, $item_Detail['category_id'], $item_Detail['item_id'], $sizeId);
...
//                    if()
    ...
    function completeOrder($requestBody = null, $paymentType = null, $response = null)
    {
        ...
        try {
            ...
            if ($this->orderTranSave($orderData, $paymentType)) {
                ...
                $this->Order->updateAll(array('payment_id' => $orderPaymentID), array('Order.id' => $orderid));
                $ds->commit($this);
                return $orderid;
            } else {
                throw new Exception("NZ Gateway Sale Error : ");
    ...
    public function proceedOrder()
    {
        ...
        if (isset($requestBody['temp_order_id']) && !empty($requestBody['temp_order_id'])) {
            ...
            if ($result) {
                $OrderDetail = $this->Order->find('first', array('fields' => array('id', 'coupon_code'), 'conditions' => array('Order.id' => $result)));
                if (!empty($OrderDetail['coupon_code'])) {
                    $CouponCode = $this->Coupon->find('first', array('conditions' => array('Coupon.coupon_code' => $OrderDetail['coupon_code'], 'Coupon.is_active' => 1, 'Coupon.is_deleted' => 0), 'fields' => array('id', 'number_can_use', 'used_count', 'discount', 'discount_type')));
                    $cudata['Coupon']['used_count'] = $CouponCode['Coupon']['used_count'] + 1;
                    $cudata['Coupon']['id'] = $CouponCode['Coupon']['id'];
                    $this->Coupon->saveCoupon($cudata);
                }
        ...
        if ($this->Order->saveOrder($orderInfo['orderdata'])) {
            ...
            if ($paymemt_type != 4) {
                ...
                if ($store_info['Store']['is_kitchen_printer'] == 1) {
                    $this->StorePrintHistory->saveStorePrintHistory($this->_getOrderItem('KitchenPrinter', null, $detail));
                }
                if ($store_info['Store']['is_receipt_printer'] == 1) {
                    $this->StorePrintHistory->saveStorePrintHistory($this->_getOrderItem('ReceiptPrinter', null, $detail));
                }
            }
        } else {
            return false;
                    ...
                    if (!empty($userDet)) {
                        ...
                        if (!empty($myOrders)) {
                            foreach ($myOrders as $o => $listOrder) {
                                ...
                                $myOrdersList[$o]['placed_date'] = date('m-d-Y h:m a', strtotime($listOrder['Order']['created']));
                                ...
                                if (!empty($listOrder['OrderItem'])) {
                                    foreach ($listOrder['OrderItem'] as $oI => $listOrderItem) {
                                        ...
                                        $FreeItem = $this->OrderItemFree->find('first', array('fields' => array('id', 'item_id', 'free_quantity'), 'conditions' => array('OrderItemFree.item_id' => $listOrderItem['Item']['id'], 'OrderItemFree.order_id' => $listOrderItem['order_id'], 'OrderItemFree.is_active' => 1, 'OrderItemFree.is_deleted' => 0)));
                            ...
                            foreach ($mySavedOrders as $o => $listOrder) {
                                ...
                                if (!empty($listOrder['OrderItem'])) {
                                    foreach ($listOrder['OrderItem'] as $oI => $listOrderItem) {
                                        ...
                                        $FreeItem = $this->OrderItemFree->find('first', array('fields' => array('id', 'item_id', 'free_quantity'), 'conditions' => array('OrderItemFree.item_id' => $listOrderItem['item_id'], 'OrderItemFree.order_id' => $listOrder['Order']['id'], 'OrderItemFree.is_active' => 1, 'OrderItemFree.is_deleted' => 0)));
...
//                                            $preferencePrice = 0;
    ...
    public function verifyAddress()
    {
        ...
        $headers = $this->apache_request_headers();
        $requestBody = file_get_contents('php://input');
        $this->Webservice->webserviceLog($requestBody, "verify_Address.txt", $headers);
        //$requestBody = '{"name_on_bell":"Ranjeet","label":"1","address":"Torrance","city":"CA","state":"CA","zipcode":"90501","country_code":"+91","phone":"233235688888888","default":false}';
        //$headers['user_id'] = 'NTI2';
        //$headers['merchant_id'] = 85;
        $responsedata = array();
        ...
        if (isset($headers['merchant_id']) && !empty($headers['merchant_id'])) {
            ...
            if (!empty($merchantCheck)) {
                ...
                if (isset($headers['user_id'])) {
                    ...
                    $zoneData = $this->Common->setZonefee($DelAddress, $requestBody['store_id'], "mob");
                    if (empty($zoneData)) {
                        $response['message'] = "Order can't be delivered at this address.";
                        $response['response'] = 0;
                        return json_encode($response);
        ...
        if (isset($headers['merchant_id']) && !empty($headers['merchant_id'])) {
            ...
            if (!empty($merchantCheck)) {
                ...
                if (!empty($requestBody['keyword'])) {
                    ...
                    $locStoreList = $this->Store->find('all', array('conditions' => array('Store.is_active' => 1, 'Store.is_deleted' => 0, 'Store.merchant_id' => $merchant_id), 'fields' => array('distance', 'id', 'store_name', 'address', 'city', 'state', 'store_logo', 'phone', 'zipcode', 'merchant_id', 'delivery_fee', 'service_fee', 'minimum_order_price', 'minimum_takeaway_price', 'is_booking_open', 'is_delivery', 'cash_on_delivery', 'is_pay_by_credit_card', 'is_express_check_out', 'deliverycalendar_limit', 'pickcalendar_limit', 'deliveryblackout_limit', 'pickblackout_limit', 'dineinblackout_limit', 'pre_order_allowed', 'calendar_limit', 'dineinblackout_limit', 'is_take_away', 'is_booking_open'), 'group' => array('Store__distance HAVING Store__distance < ' . $requestBody['distance'])));
                } else {
                    $responsedata['data'] = array();
                    $responsedata['response'] = 1;
