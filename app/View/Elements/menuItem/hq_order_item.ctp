                    ...
                    if ($listItem['is_seasonal_item'] == 1) {
                        if ($current_date >= $starDate && $current_date <= $endDate) {
                            ?>
                                ...
                                <li class='itemName'><?php echo $this->Html->link(wordwrap($listItem['name'],15,"\n",false), 'javascript:void(0)', array('class' => 'item_link parent_category_id' . $category_id, 'rel' => $item_id, 'id' => $id_name, 'store_id' => $store_id, 'item_parent_id' => $category_id, 'size-type' => $list['Category']['is_sizeonly'])); ?></li>
                        <?php } else {
                        }
                        ?>   
                            <?php } else { ?>
                                <li class='itemName'><?php echo $this->Html->link(wordwrap($listItem['name'],15,"\n",flase), 'javascript:void(0)', array('class' => 'item_link parent_category_id' . $category_id, 'rel' => $item_id, 'id' => $id_name, 'store_id' => $store_id, 'item_parent_id' => $category_id, 'size-type' => $list['Category']['is_sizeonly'])); ?></li>
